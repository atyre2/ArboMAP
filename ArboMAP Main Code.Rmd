...
simplifynames <- function(priornames=NULL) {
  ...
  # remove numbers
  priornames <- gsub(pattern="1", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="2", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="3", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="4", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="5", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="6", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="7", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="8", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="9", replacement="", x=priornames, fixed=TRUE)
  priornames <- gsub(pattern="0", replacement="", x=priornames, fixed=TRUE)
...
packages <- c("reshape2", "ggplot2", "gridExtra",
              ...
              "broom","mapproj", "Hmisc", "pROC", "parallel",
              "tigris", "knitr")
...
# model formulas
modelformulas <- c("cub-fx-nonanom"= "anycases ~ 0 + district + cenppr + cenyear + s(lag, by=var1, bs='cr', fx=TRUE, k=6) + s(lag, by=var2, bs='cr', fx=TRUE, k=6)",
                   "cub-fx-anom"   = "anycases ~ 0 + district + cenppr + cenyear + s(lag, by=anom_var1, bs='cr', fx=TRUE, k=6) + s(lag, by=anom_var2, bs='cr', fx=TRUE, k=6) + s(doy, bs='cr')",
                   "cub-sv-nonanom"= "anycases ~ 0 + district + cenppr + cenyear + te(lag, doymat, by=var1, bs='cr', fx=TRUE, k=6) + te(lag, doymat, by=var2, bs='cr', fx=TRUE, k=6)",
                   "cub-sv-anom"   = "anycases ~ 0 + district + cenppr + cenyear + te(lag, doymat, by=anom_var1, bs='cr', fx=TRUE, k=6) + te(lag, doymat, by=anom_var2, bs='cr', fx=TRUE, k=6) + s(doy, bs='cr')",
                   "tp-fx-nonanom" = "anycases ~ 0 + district + cenppr + cenyear + s(lag, by=var1, bs='tp') + s(lag, by=var2, bs='tp')",
                   "tp-fx-anom"    = "anycases ~ 0 + district + cenppr + cenyear + s(lag, by=anom_var1, bs='tp') + s(lag, by=anom_var2, bs='tp') + s(doy, bs='cr')",
                   "tp-sv-nonanom" = "anycases ~ 0 + district + cenppr + cenyear + te(lag, doymat, by=var1, bs='tp') + te(lag, doymat, by=var2, bs='tp')",
                   "tp-sv-anom"    = "anycases ~ 0 + district + cenppr + cenyear + te(lag, doymat, by=anom_var1, bs='tp') + te(lag, doymat, by=anom_var2, bs='tp') + s(doy, bs='tp')")

## run all listed models
#modelnames <- names(modelformulas)

## or choose just some selection
modelnames <- c("tp-sv-anom")

# where are the human data located?
# example of a file with directories an drive:
humandatafile <- ".\\human case data\\19-08-29 - LA human data.csv"
...
mosqfile <- ".\\mosquito data\\19-08-29 - reconciled mosquito data.csv"
...
# simplify name
district_shapes <- counties("Louisiana", cb=TRUE)
...
wnv$year <- format(wnv$year)
wnv$poolpositiverate <- wnv$numpos / wnv$total
```

```{r humandata, include=FALSE, echo=TRUE}

# import data
# load the pre-2012 summarized data
pre2012 <- read.csv("C:\\home\\work\\davis\\ArboMAP\\human case data\\19-09-06 - reconciled EOY updates.csv", stringsAsFactors=FALSE)
pre2012 <- melt(pre2012, id.vars=c("parish", "year"))
pre2012 <- pre2012[!is.na(pre2012$value),]
pre2012$week <- as.numeric(substr(pre2012$variable, 2, 4))
pre2012$creationdate <- as.Date(paste(pre2012$year, "-01-01", sep=""), "%Y-%m-%d")
pre2012$creationdate <- pre2012$creationdate + (pre2012$week - 1) * 7 + 3
pre2012$creationdate <- format(pre2012$creationdate, "%m/%d/%Y")
pre2012$week <- NULL
pre2012$variable <- NULL
pre2012 <- as.data.frame(lapply(pre2012, rep, pre2012$value))
pre2012$value <- NULL
pre2012$year <- NULL
pre2012$district <- pre2012$parish
pre2012$parish <- NULL

# import post-2012 data
human <- read.csv(humandatafile, stringsAsFactors=FALSE)
human <- bind_rows(human, pre2012)
begrow <- nrow(human)
human$chardate <- as.character(levels(human$creationdate))[as.numeric(human$creationdate)]
human$date <- as.Date(human$creationdate, "%m/%d/%Y")
human$creationyear <- as.numeric(format(human$date, "%Y"))
human$creationmonth <- as.numeric(format(human$date, "%m"))

# figure out which are the max and min human years
minobshumanyear <- min(human$creationyear, na.rm=TRUE)
maxobshumanyear <- max(human$creationyear, na.rm=TRUE)
...
```{r humandata2, echo=FALSE, warnings=FALSE, include=FALSE}
...
totaldailyextr <- expand.grid(district=unique(dailyextr$district),
                              date=seq(from=as.Date(paste((minobshumanyear-1), "-01-01", sep=""),
                                                    "%Y-%m-%d"),
...
# include mosquito data
fullcasemat$year <- format(fullcasemat$weekstartdate, "%Y")
fullcasemat <- left_join(fullcasemat, wnv, by="year")

# calculate missing MIRsummarystat
fullcasemat$cenppr <- fullcasemat$poolpositiverate
fullcasemat$cenppr[is.na(fullcasemat$cenppr)] <- mean(fullcasemat$poolpositiverate, na.rm=TRUE)
# put in a censoring year
fullcasemat$cenyear <- format(fullcasemat$weekstartdate, "%Y")
fullcasemat$cenyear[fullcasemat$cenyear %in% wnv$year] <- -1
fullcasemat$cenyear <- factor(fullcasemat$cenyear)
fullcasemat <- ungroup(fullcasemat)
```

```{r, echo=FALSE, include=TRUE}
# plot relationship of cases to MIGR
tempdf <- group_by(fullcasemat, year)
tempdf <- dplyr::summarise(tempdf,
                           totrisk = sum(anycases, na.rm=TRUE),
                           MIGR    = sum(poolpositiverate))
tempdf$totrisk[tempdf$year == 2019] <- 20
thisplot <- ggplot(tempdf) + geom_smooth(aes(x=MIGR, y=totrisk), method=lm) +
  geom_label(aes(x=MIGR, y=totrisk, label=year)) +
  ggtitle("MIGR vs. total annual risk") +
  xlab("estimated MIGR") + ylab("total risk in year") +
...
tempdf$newwe <- (tempdf$week - (humanminobsweek-0)) / ((humanmaxobsweek+0) - (humanminobsweek-0) + 1)
...
thisplot <- ggplot() + geom_line(data=tempdf, aes(x=newdate, y=obs, group=model)) +
  ...
  xlab("") + ylab("") +
  scale_x_continuous(breaks=seq(from=minobshumanyear,to=(maxobshumanyear+1),by=1),
                     labels=c(seq(from=minobshumanyear, to=maxobshumanyear, by=1), ""),
                     limits=c((minobshumanyear+0.125),(maxobshumanyear+1))) +
...
thisplot2 <- ggplot(data=tempdf2) +
  ...
  scale_color_manual("", values = mycolors) +
  scale_linetype_manual("", values = mylinetypes) +
  ggtitle(paste("Statewide model predictions in ", format(maxdesiredhumandate, "%Y"), sep="")) +
  xlab("") + ylab("") +
...
```{r, echo=FALSE, include=TRUE, fig.width=7, fig.height=7}
thisplot3 <- ggplot() + geom_jitter(data=tempdf, aes(x=obs, y=est)) +
  facet_wrap(~model, ncol=2) + 
  #scale_color_manual(values = mycolors) +
  ggtitle("Statewide proportion of districts positive") +
...
projected_districts <- district_shapes
  ...
  for (curplot in 1:length(thisplot)) {
    ...
    if (is.null(thissubplot$main)) {
      ...
      tempdf <- data.frame(x=thissubplot$x,
                           y=thissubplot$fit,
                           se=thissubplot$se)
      write.csv(tempdf, file = paste("C:\\home\\work\\davis\\ArboMAP\\various graphics\\louisiana_", modelcount, ".csv", sep="")) 
...
write.csv(fullcasemat, file="C:\\home\\work\\davis\\LA SD arbomap comparison\\lafullcasemat.csv")
```
