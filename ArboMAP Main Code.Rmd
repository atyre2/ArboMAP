...
modelnames <- c("tp-fx-nonanom", "tp-fx-anom", "tp-sv-nonanom", "tp-sv-anom")
...
tempdf$newwe <- (tempdf$week - (humanminobsweek-5)) / ((humanmaxobsweek+5) - (humanminobsweek-5) + 1)
...
thisplot3 <- ggplot() + geom_jitter(data=tempdf, aes(x=obs, y=est)) +
  ...
  theme(panel.grid.minor.x=element_blank()) +
  theme(legend.position="bottom") +
  geom_abline(slope=1, intercept=0, color="red", linetype=2, alpha=0.2) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
        legend.key = element_blank())
plot(thisplot3)
```
